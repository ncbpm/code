...
public class PullDataMaintainServiceImpl extends CMAlgorithmBaseFramework implements IPullDataMaintainService {
    @Override
    ...
    @Override
    public boolean fetchDataForCostTran(FetchParamVO paramvo, List<String> cycObjMaterialLst, Set<String> iaMaterialSet)
        ...
        billTypeMap.put(Integer.valueOf(CMBillEnum.MATEROUT.getEnumValue().getValue()), tranpks);
        ...
        // 设置为成本结转
        paramvo.setIfetchobjtype(Integer.valueOf(FetchDataObjEnum.MATERIALOUT_COSTTRAN.getEnumValue().getValue()));
        // 账簿
        String acountBook = IAAdapter.getAcountBookParam4ICInteface(paramvo.getPk_org());
        // 成本域
        Set<String> costDomain = BDAdapter.getAllCostRegionIDByPkOrg(paramvo.getPk_org());
        paramvo.setAcountBook(acountBook);
        paramvo.setCostDomain(costDomain);
        paramvo.setIfetchscheme(fetchschema);
        paramvo.setNday(result[0].getNday());
        PullDataErroInfoVO[] errorVOs = this.getCheckAndFetchComom(paramvo, false, cycObjMaterialLst, iaMaterialSet);
        if (CMArrayUtil.isEmpty(errorVOs)) {
            ...
            for (PullDataErroInfoVO pullDataErroInfoVO : errorVOs) {
                ...
                msg.append("MaterialID：[" + pullDataErroInfoVO.getCmaterialvid() + "]");
                msg.append("CostCenterID：[" + pullDataErroInfoVO.getCcostcenterid() + "]");
    ...
    private void getBillTypeMap(FetchParamVO paramvo) {
        ...
        if (FetchDataSchemaEnum.WEEKSCHEMA.getEnumValue().getValue().equals(paramvo.getIfetchscheme().toString())
                && paramvo.getIfetchobjtype() != FetchDataObjEnum.ACT.toIntValue()) {
            Integer fetchType = paramvo.getIfetchobjtype();
            // 查询取数类型和交易类型的对照关系
            FetchSetItemVO[] fetchSetItems = null;
            try {
                fetchSetItems =
                        NCLocator.getInstance().lookup(IFetchSetPubQueryService.class)
                        .getFetchSetItems(fetchType, paramvo.getPk_org(), paramvo.getPk_group());
            }
            catch (BusinessException e) {
                ExceptionUtils.wrappException(e);
            }
            if (CMArrayUtil.isEmpty(fetchSetItems)) {
                ExceptionUtils.wrappBusinessException(FetchDataLangConst.getNO_TRANSTYPE_DEFINED());
                return;
            }
            Map<Integer, List<String>> billTypeMap = PullDataUtil.createBillTypeMap(fetchSetItems, fetchType);
            // 设置取数参数
            paramvo.setBillTypeMap(billTypeMap);
        }
    }
    ...
    public Integer fetchData(FetchParamVO paramvo) throws BusinessException {
        try {
            ...
            if (isCoverData) {
                ...
                ICheckAndFetch type = fatory.createFetchTypeFactory(ifetchobjtype, null, null);
                type.deleteBill(paramvo);
            }

            // 该操作包括保存数据
            PullDataErroInfoVO[] errorVOs = this.getCheckAndFetchComom(paramvo, false, null, null);
            Integer status = null;
            for (PullDataErroInfoVO infoVO : errorVOs) {
                if (infoVO.getImestype() == null || infoVO.getImestype().equals(CMMesTypeEnum.ERROR.toIntValue())) {
                    return CMMesTypeEnum.ERROR.toIntValue();
                }
                else if (infoVO.getImestype().equals(CMMesTypeEnum.WARN.toIntValue())) {
    ...
    private void checkTS(FetchParamVO param) {
        ...
        sql.append(param.getPk_org() + "'and cperiod='" + param.getDaccountperiod() + "' and pulldatatype=");
        sql.append(param.getPulldatatype() + " and ifetchobjtype=" + param.getIfetchobjtype() + " and ifetchscheme="
                + param.getIfetchscheme());
        BaseDAO dao = new BaseDAO();
        Map<String, String> tsMap = null;
        try {
            tsMap = (Map<String, String>) dao.executeQuery(sql.toString(), new ResultSetProcessor() {
                ...
                public Object handleResultSet(ResultSet rs) throws SQLException {
                    ...
                    while (rs.next()) {
                        tsMap.put(rs.getInt(1) + rs.getString(2) + rs.getString(3) + rs.getString(4) + rs.getString(5),
                                rs.getString(6));
                    }
                    return tsMap;
                }
        ...
        for (PullDataStateVO vo : vos) {
            String ts =
                    tsMap.get((vo.getCbilltype() == null ? "0" : vo.getCbilltype()) + vo.getCcostcenterid()
                            + vo.getCtranstypeid() + vo.getDbegindate() + vo.getDenddate());
            if (vo.getTs() == null || !ts.equals(vo.getTs().toString())) {
                ExceptionUtils.wrappBusinessException(FetchDataLangConst.getNEW_STATUS());
            }
        }
    }

    @Override
    public Map<FetchKeyVO, Integer> allCancel(FetchParamVO paramvo) throws BusinessException {
        ...
        return (Map<FetchKeyVO, Integer>) this.doCancelExecute(pkOrg, period, "doAllCancel", parameterTypes, param);
    }

    public Map<FetchKeyVO, Integer> doAllCancel(FetchParamVO paramvo) throws BusinessException {
        try {
            // 获取取数状态信息
            ...
            for (String pk_org : orgSet) {
                if (NCLocator.getInstance().lookup(ICostDataCloseForCMService.class)
                        .isCloseAccount(paramvo.getPk_group(), pk_org, new String[] {
            ...
            // 检查是否有启用模块
            PullDataUtil.moduleEnable(this.getFetchDataMainSelectVO(fetchAllDataMap));

            for (Entry<FetchKeyVO, List<PullDataStateVO>> entry : fetchAllDataMap.entrySet()) {
                ...
                if (ifetchType.equals(FetchDataObjEnum.ISSTUFF.toIntValue())) {
                    // 取消取数---其他出入库消耗单
                    resultMap.put(keyVO, this.cancelFetchIAStuffData(pk_group, pk_org, cperiod));
                    continue;
                }

                PullDataStateVO[] allVO = entry.getValue().toArray(new PullDataStateVO[0]);
                FetchParamVO fetchParamvo = this.getParamVO(pk_group, pk_org, cperiod, allVO);
                // 取消取数
    ...
    private Map<FetchKeyVO, List<PullDataStateVO>> getFetchAllDataMap(FetchParamVO paramvo) throws BusinessException {
        if (CMMapUtil.isEmpty(paramvo.getFetchAllDataMap())) {
            List<AggFetchSetVO> fetchSetLst = new ArrayList<AggFetchSetVO>();
            PullDataStateVO pullParamvo =
                    this.getPullParamVO(paramvo.getPk_group(), paramvo.getPk_org(), paramvo.getDaccountperiod());
            Map<FetchKeyVO, UIAllDataVO> vosMap =
                    NCLocator.getInstance().lookup(IPullDataQueryService.class).getVos4UiDataSet(pullParamvo);
            if (CMMapUtil.isNotEmpty(vosMap)) {
                Map<FetchKeyVO, List<PullDataStateVO>> fetchAllDataMap =
                        new HashMap<FetchKeyVO, List<PullDataStateVO>>();
                for (Entry<FetchKeyVO, UIAllDataVO> entry : vosMap.entrySet()) {
                    UIAllDataVO vo = entry.getValue();
                    if (CMArrayUtil.isNotEmpty(vo.getVos())) {

                        fetchAllDataMap.put(entry.getKey(), Arrays.asList(vo.getVos()));
                    }
                    if (CMArrayUtil.isNotEmpty(vo.getFetchSetAggVo())) {
                        fetchSetLst.addAll(Arrays.asList(vo.getFetchSetAggVo()));
                    }
                }
            ...
            if (CMCollectionUtil.isEmpty(fetchSetLst)) {
                ExceptionUtils.wrappBusinessException(FetchDataLangConst.getERR_NORELATIONFETCHSET());
            }
    ...
    private PullDataStateVO getPullParamVO(String pk_group, String pk_org, String cperiod) {

        PullDataStateVO paramvo = new PullDataStateVO();
        paramvo.setPk_group(pk_group);
        paramvo.setPk_org(pk_org);
        // 根据财务组织查询所有关联工厂
        paramvo.setPk_orgs(new String[] {
                paramvo.getPk_org()
        });
        paramvo.setIfetchobjtype(FetchDataObjEnum.ALL); // 取数对象： 1 材料出，2产成品入 3 废品 4 作业
    ...
    public Integer doCancel(FetchParamVO paramvo) throws BusinessException {
        ...
        // 2.根据取数类型删除数据
        IAllCancel allcancel = new AllCancelFactroy().createAllCancelFactory(paramvo.getIfetchobjtype());
     ...
     */
    @Override
    public PullDataErroInfoVO[] getErroInfoVos(FetchParamVO paramvo) throws BusinessException {
        try {
            // 获取取数状态信息
            ...
            // 检查是否有启用模块
            PullDataUtil.moduleEnable(this.getFetchDataMainSelectVO(paramvo.getFetchSelectDataMap()));

            List<PullDataErroInfoVO> errovolst = new ArrayList<PullDataErroInfoVO>();
            for (Entry<FetchKeyVO, List<PullDataStateVO>> entry : paramvo.getFetchSelectDataMap().entrySet()) {
                List<PullDataStateVO> lst = entry.getValue();
                // 1得到主页面表体选择行的vo
                PullDataStateVO[] pullDataStateVOs = lst.toArray(new PullDataStateVO[0]);
                // 2 查询 后台进行检查并返回前台
                FetchParamVO fetchParamvo =
                        this.getParamVO(paramvo.getPk_group(), entry.getKey().getPk_org(), paramvo.getDaccountperiod(),
                                pullDataStateVOs);
                PullDataErroInfoVO[] errovos = this.doEachCheckErro(fetchParamvo);
    ...
    public PullDataErroInfoVO[] doEachCheckErro(FetchParamVO paramvo) throws BusinessException {
     ...
     * @return 错误信息
     */
    private PullDataErroInfoVO[] getCheckAndFetchComom(FetchParamVO paramvo, boolean isCheckFlag,
            List<String> cycObjMaterialLst, Set<String> iaMaterialSet) throws BusinessException {
        Integer ifetchobjtype = paramvo.getIfetchobjtype();
        PullDataErroInfoVO[] errorVOs = null;
        try {
            // 根据取数类型<材料出库单/产成品入库单/完工报告或者工序完工报告>调用检查方法
            FetchTypeFactroy fatory = new FetchTypeFactroy();
            ICheckAndFetch type = fatory.createFetchTypeFactory(ifetchobjtype, cycObjMaterialLst, iaMaterialSet);
    ...
    public Object[] fetchAllData(FetchParamVO paramvo) throws BusinessException {
        ...
        return (Object[]) this.doExecute(pkOrg, period, "dofetchAllData", parameterTypes, param);
    }

    private void dealFetchSelectData(FetchParamVO paramvo) {
        Map<FetchKeyVO, List<PullDataStateVO>> fetchSelectDataMap = paramvo.getFetchSelectDataMap();
        Map<FetchKeyVO, List<PullDataStateVO>> fetchAllDataMap = paramvo.getFetchAllDataMap();
        ...
        if (CMMapUtil.isNotEmpty(allDataMap)) {
            for (Entry<FetchKeyVO, List<PullDataStateVO>> entry : allDataMap.entrySet()) {
    ...
    public Object dofetchAllData(FetchParamVO paramvo) throws BusinessException {
        try {
            ...
            for (Entry<FetchKeyVO, List<PullDataStateVO>> entry : paramvo.getFetchSelectDataMap().entrySet()) {
                ...
                PullDataStateVO[] pullDataStateVOs = entry.getValue().toArray(new PullDataStateVO[0]);
                // 最终取数参数信息
                FetchParamVO paramNewVO = this.getParamVO(pk_group, pk_org, cperiod, pullDataStateVOs);
                ...
                if (!paramNewVO.isFetchBefore()) {
                    ...
                    List<PullDataStateVO> lst = paramvo.getFetchAllDataMap().get(keyVO);
                    if (CMCollectionUtil.isEmpty(lst)) {
                        continue;
                    }

                    PullDataStateVO[] allVO = lst.toArray(new PullDataStateVO[0]);
                    FetchParamVO allParamvo = this.getParamVO(pk_group, pk_org, cperiod, allVO);
                    paramNewVO.setAllFetchParamVo(allParamvo);
                    // 设置取数状态为取过数
                    PullDataUtil.setFetchedStatusTOallVO(allVO, pullDataStateVOs);
                ...
                paras[0] = paramNewVO;
                // try {
                Object ret = CMTProxySrv.delegate_RequiresNew(this, method, paras);
                if (ret != null) {
                    errorStatusMap.put(keyVO, (Integer) ret);
                }
                else {
                    errorStatusMap.put(keyVO, CMMesTypeEnum.SUCCESS.toIntValue());
                }
                // }
                // catch (Exception e) {
    ...
    private void showMsg(Map<FetchKeyVO, Integer> errorStatusMap) {
        ...
        for (Entry<FetchKeyVO, Integer> entry : errorStatusMap.entrySet()) {
            String ifetchobjName = this.getFetchObjName(entry.getKey().getIfetchobjtype());
            if (entry.getValue().equals(CMMesTypeEnum.ERROR.toIntValue())) {
                // 取数对象:取数失败，请进行检查操作获取失败原因。
                errMsg.append(FetchDataLangConst.getCOMBOX_FETCHOBJ() + "[" + ifetchobjName + "]");
    ...
    private Method getMethod(Object target) {
        ...
        Method method = null;
        try {
            method = target.getClass().getDeclaredMethod("fetchData", parameterTypes);
        }
        catch (Exception e) {
            ExceptionUtils.wrappException(e);
        }
     ...
     * @return 参数vo
     */
    private FetchParamVO getParamVO(String pk_group, String pk_org, String cperiod, PullDataStateVO[] pullDataStateVOs)
            throws BusinessException {
        ...
        for (PullDataStateVO vo : pullDataStateVOs) {
            ...
            if (CMStringUtil.isNotEmpty(vo.getCcostcenterid()) && !costCenteridList.contains(vo.getCcostcenterid())) {
            ...
            if (Integer.valueOf(FetchDataSchemaEnum.PERIODSCHEMA.getEnumValue().getValue()).compareTo(
                ...
                if (!paramvo.getBillTypeMap().containsKey(vo.getCbilltype())) {
                    ...
                    List<String> list = paramvo.getBillTypeMap().get(vo.getCbilltype());
    ...
    private boolean isFethed(PullDataStateVO[] paramvos) throws BusinessException {
     ...
     * 得到主页面表体选择行的vo
     **/
    private PullDataStateVO[] getFetchDataMainSelectVO(Map<FetchKeyVO, List<PullDataStateVO>> fetchSelectDataMap) {
        List<PullDataStateVO> lst = new ArrayList<PullDataStateVO>();
        for (Entry<FetchKeyVO, List<PullDataStateVO>> entry : fetchSelectDataMap.entrySet()) {
            lst.addAll(entry.getValue());
        }
        return lst.toArray(new PullDataStateVO[0]);
    }

    private Integer cancelFetchIAStuffData(String pk_group, String pk_org, String cperiod) throws BusinessException {
        NCLocator.getInstance().lookup(IIastuffMaintainService.class).cancelFetchData(pk_group, pk_org, cperiod);
        return CMMesTypeEnum.SUCCESS.toIntValue();
    }
}
