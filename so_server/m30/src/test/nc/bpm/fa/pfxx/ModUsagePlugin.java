...
public class ModUsagePlugin extends AbstractPfxxPlugin{
	...
	protected Object processBill(Object vo, ISwapContext swapContext,
		...
		String billPK = PfxxPluginUtils.queryBillPKBeforeSaveOrUpdate(swapContext
				.getBilltype(), headVO.getBill_code(), swapContext.getOrgPk());

		// 如果导入过
		if(null != billPK && billPK.length() >0){
			// 查看配置文件信息是否允许导入重复数据
	 		if (swapContext.getReplace().equalsIgnoreCase("N")){
	 			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("farule_0","02012060-0037")/*@res "不允许重复导入单据，如果想更新已导入单据，请把数据文件的replace标志设为‘Y’！"*/);
	 		}
	 		// 设置主键
	 		headVO.setPrimaryKey(billPK);
	 		// 查询数据库中是否存在该单据
	 		AlterVO alterVO = ArrayUtils.getFirstElem(AMProxy.lookup(IAlterImport.class).queryAlterVO(billPK));
	 		if(null != alterVO){
	 			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("farule_0","02012060-0038")/*@res "该单据已经导入，并且已经审核，不允许重复导入。"*/);
	 		}
		}
 		...
 		//6. 生成变动单据
 		AlterVO returnBillVO = AMProxy.lookup(IAlterImport.class).importInsert(billVO, (List<String>) CollectionUtils.convertToList(showKeys));
    ...
    private String[] getShowKeyList(AlterVO billVO) throws BusinessException {
		...
		if (headVO == null || bodyVOs == null || bodyVOs.length == 0) {
			ExceptionUtils.asBusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("farule_0","02012060-0039")/*@res "变动单表头或者表体没有数据, 请检查数据文件 \n "*/);
		}
		...
		for (AlterBodyVO bodyVO : bodyVOs) {
			...
			if (sheetVOs == null || sheetVOs.length == 0) {
				ExceptionUtils.asBusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("farule_0","02012060-0040")/*@res "altersheetvos为空, 请检查数据文件 \n "*/);
			}
	 ...
	 */
	private AssetVO[] queryCardVOs(AlterVO billVO, String[] showKeys) throws BusinessException {
		String[] pk_cards = VOManager.getAttributeValueArray(billVO.getChildrenVO(), AssetFieldConst.PK_CARD);

		AlterHeadVO headVO = (AlterHeadVO) billVO.getParentVO();
		String whereSQL = " fa_card.pk_card in (" + StringTools.buildStringUseSpliterWithQuotes(pk_cards, ",") + ") " +
		...
		//添加查询字段
		List<String> keysList = (List<String>) CollectionUtils.convertToList(showKeys);
		...
		if (assetVOs == null || assetVOs.length ==0) {
			ExceptionUtils.asBusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("farule_0","02012060-0041")/*@res " 没有查询到任何一张卡片, 请检查 \n "*/);
		}
	 ...
	 */
	private void checkBill(AlterVO billVO, AssetVO[] assetVOs, String[] showKeys) throws BusinessException {
		String[] pk_cards = VOManager.getAttributeValueArray(billVO.getChildrenVO(), AssetFieldConst.PK_CARD);
		...
		if (cards.size() != 0) {
           ...
           errorMsg.append(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID(
                   "farule_0", 
                   "02012060-0043", 
                   null, 
                   new String[]{StringTools.buildStringUseSpliterWithQuotes(cards.toArray(new String[0]), ",")}));
		}

		//2. 校验输入的卡片的变动项是否正确
		//所有卡片的字段名称
		List<String> cardItemKeys = Arrays.asList(new AssetVO().getAttributeNames());
		//有问题的变动的卡片项目
		...
		if (errorItemKeys.size() != 0) {
			...
			errorMsg.append(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID(
			        ...
			        "02012060-0044", 
			        null, 
			        new String[]{StringTools.buildStringUseSpliterWithQuotes(
			                errorItemKeys.toArray(new String[0]), ",")}));
		...
		for (AlterBodyVO bodyVO : (AlterBodyVO[])billVO.getChildrenVO()) {
			...
			for (AlterSheetVO sheetVO : bodyVO.getAltersheetvos()) {
				...
				//变动前 和 卡片都不为空
				if (cardVO.getAttributeValue(itemKey) != null && alter_before != null ) { 
					
					// 匹配值类型
					try {
						UFDouble cardValue = new UFDouble(sheetVO.getOld_content());
						UFDouble oldValue = new UFDouble(alter_before.toString());
						...
						try {
							Integer cardInt = new Integer(sheetVO.getOld_content());
							Integer oldInt = new Integer(alter_before.toString());
							//不相等校验
							if (oldInt != cardInt) {
							    /*@res "卡片主键为[{0}]对应的变动卡片项[{1}]的值与卡片不相同, 不能导入。 \n"*/
	                            errorMsg.append(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID(
							...
							//不相等校验
							if (!cardVO.getAttributeValue(itemKey).toString().equals(alter_before.toString())) {
							    /*@res "卡片主键为[{0}]对应的变动卡片项[{1}]的值与卡片不相同, 不能导入。 \n"*/
	                            errorMsg.append(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID(
	                                    "farule_0", 
	                                    "02012060-0046", 
	 ...
	 * @param cardVOs
	 */
	private void setBillVOValue(AlterVO billVO, AssetVO[] cardVOs, String[] showKeys) throws BusinessException {
		AlterBodyVO[] bodyVOs = (AlterBodyVO[]) billVO.getChildrenVO();
		...
		for (AlterBodyVO bodyVO : bodyVOs) {
			...
			for (String showKey : showKeys) {
				...
				if (sheetCodes.keySet().contains(showKey)) {
					IGeneralAccessor generalAccessor = FABDDataManager.createGeneralAccessor(showKey);
					//NullGeneralAccessor 类型说明不是档案类型
					if (generalAccessor instanceof NullGeneralAccessor) {
						//变动前
						bodyVO.setAttributeValue(sheetVO.getItem_code()+ AlterKeyConst.BEFORE_SUFFIX,
								sheetVO.getOld_content());

						//变动后
						bodyVO.setAttributeValue(sheetVO.getItem_code()+ AlterKeyConst.AFTER_SUFFIX,
								sheetVO.getNew_content());
						
						// 变动部分
						UFDouble newValue = new UFDouble(sheetVO.getNew_content());
						UFDouble oldValue = new UFDouble(sheetVO.getOld_content());
						bodyVO.setAttributeValue(sheetVO.getItem_code()+ AlterKeyConst.ALTER_SUFFIX,
                               UFDoubleUtils.sub(newValue, oldValue));
					} else {
						//1. 通过编码来取(用的比较多) 这个是走缓存的
						Map<String, String> codeAndPkMaps = FABDDataManager.getBDPKByCodes(showKey,
								new String[] {sheetCodes.get(showKey).getOld_content(), sheetCodes.get(showKey).getNew_content() },
								((AlterHeadVO) billVO.getParentVO()).getPk_org());

						if (codeAndPkMaps != null && codeAndPkMaps.size() != 0) {
							//变动前PK
							String before = codeAndPkMaps.get(sheetVO.getOld_content());
							//变动后PK
							String after = codeAndPkMaps.get(sheetVO.getNew_content());

							//使用部门单独处理， 要转化成link_key存储
							if (showKey.equals(AssetFieldConst.PK_USEDEPT)) {
								String[] pk_depts = new String[]{before, after};
							...
							bodyVO.setAttributeValue(sheetVO.getItem_code()+ AlterKeyConst.BEFORE_SUFFIX, before);
							bodyVO.setAttributeValue(sheetVO.getItem_code()+ AlterKeyConst.AFTER_SUFFIX, after);

						} else {
							//2. 通过名称来取
							Map<String, String> nameAndPkMaps = FABDDataManager.getBDPKByNames(showKey,
									new String[] {sheetCodes.get(showKey).getOld_content(), sheetCodes.get(showKey).getNew_content()},
									((AlterHeadVO) billVO.getParentVO()).getPk_org());
							if (nameAndPkMaps != null && nameAndPkMaps.size() != 0) {
								//变动前PK
								String before = nameAndPkMaps.get(sheetVO.getOld_content());
								//变动后PK
								String after = nameAndPkMaps.get(sheetVO.getNew_content());
			...
			if (isAlter((List<String>) CollectionUtils.convertToList(showKeys))) {
				UFDouble after_localvalue = (UFDouble) bodyVO.getAttributeValue(AlterKeyConst.LOCALORIGINVALUE_AFTER);
				UFDouble after_salvage = (UFDouble) bodyVO.getAttributeValue(AlterKeyConst.SALVAGE_AFTER);
				UFDouble after_salvarate = UFDoubleUtils.div((UFDouble) bodyVO.getAttributeValue(AlterKeyConst.SALVAGERATE_AFTER), new UFDouble(100));
				
				if (!bodyVO.getAttributeValue(AlterKeyConst.SALVAGE_BEFORE)
						.equals(bodyVO.getAttributeValue(AlterKeyConst.SALVAGE_AFTER))) {
					if (UFDoubleUtils.isNullOrZero(after_salvage)) {
						bodyVO.setAttributeValue(AlterKeyConst.SALVAGERATE_AFTER, UFDouble.ZERO_DBL);
					} else {
						if (!UFDoubleUtils.isNullOrZero(after_localvalue)) {
                           bodyVO.setAttributeValue(AlterKeyConst.SALVAGERATE_AFTER,
                                   UFDoubleUtils.div(after_salvage, after_localvalue));
                       }
					}
				} else {
					if (null != bodyVO.getAttributeValue(AlterKeyConst.SALVAGERATE_AFTER)
							&& !bodyVO.getAttributeValue(AlterKeyConst.SALVAGERATE_AFTER) 
							.equals(bodyVO.getAttributeValue(AlterKeyConst.SALVAGERATE_AFTER))) {
						bodyVO.setAttributeValue(AlterKeyConst.SALVAGE_AFTER, UFDoubleUtils.multiply(after_localvalue, after_salvarate));
					} else {
						bodyVO.setAttributeValue(AlterKeyConst.SALVAGE_AFTER, UFDoubleUtils.multiply(after_localvalue, after_salvarate));
					}
				}
		...
		String pk_group = (String) billVO.getParentVO().getAttributeValue(AlterHeadVO.PK_GROUP);
		String pk_org = (String) billVO.getParentVO().getAttributeValue(AlterHeadVO.PK_ORG);
		// 调用单据号接口单据号
   	// 获取单据号管理服务
		IBillcodeManage billCodeManager = AMProxy.lookup(IBillcodeManage.class);

		// 取得新的单据号
		String billCode = billCodeManager.getPreBillCode_RequiresNew(BillTypeConst.ALTER, pk_group, pk_org);
		((AlterHeadVO)billVO.getParentVO()).setBill_code(billCode);

		if (notFitFields.size() != 0) {
			ExceptionUtils.asBusinessException(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("farule_0","02012060-0049")/*@res "数据文件xml中altersheetvos标签的item_code值, 在卡片中不存在，请重新确认. \n "*/);
		}
	}
	 ...
	 * @return
	 */
	private Map<String, String> createUsedeptScaleVOs(String[] pk_depts) throws BusinessException {
		...
		DeptScaleVO[] deptVOs = UseDeptScaleUtils.insert(deptScaleVOs.toArray(new DeptScaleVO[0]));
