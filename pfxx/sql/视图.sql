...
Where Bd_Bankaccsub.Pk_Bankaccbas In
  ...
  AND pk_org         = '0001A110000000000DDM' )
   ...
   COMMENT ON TABLE "NC06192"."VIEW_BPM_BANKACCSUB"  IS '--凭证表单中查询银行中使用-对应于NC-银行账户-集团';
--------------------------------------------------------
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FORCE VIEW "NC06192"."VIEW_BPM_card" ("资产编码", "资产名称", "资产类别编码", "资产类别名称", "规格", "型号", "设备主键", "存放地点", "生产厂家", "开始使用日期", "原币原值", "净值", "使用月限", "使用部门", "部门编码") AS 
...
--------------------------------------------------------
  ...
  select h.pk_org,h.vcardno as ccardno, case when vbillstatus=1 then'启用'else '注销' end AS CardStatus,h.vproject,pro.ph_code,pro.ph_name,
...
--------------------------------------------------------
  ...
  Select P.Crowno Crowno,--序号,
    Po_Order.Vbillcode Vbillcode,--订单编号,
    Bd_Supplier.Name Supplier_Name,--供应商,
    Bd_Supplier.Code Supplier_Code,--供应商编码,
    P.Iaccounttermno Iaccounttermno,--期账号,
    Bd_Payperiod.Name  Payperiod_Name,--起算依据,
    P.Dbegindate Dbegindate,--起算日期,
    P.Iitermdays Iitermdays,--账期天数,
    P.Denddate Denddate,--期账到期日,
    P.Bpreflag Bpreflag,--预付款,
    P.Nrate Nrate,--比例,
    P.Norigmny Norigmny,--金额,
    P.Naccumpayapporgmny Naccumpayapporgmny,--累计付款申请金额,
    P.Naccumpayorgmny Naccumpayorgmny,--累计付款金额,
    P. Pk_Order_Payplan Pk_Order_Payplan,
    Org_Financeorg.Name Org_Name ,--组织
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FORCE VIEW "NC06192"."VIEW_BPM_REGISTE_BPM" ("单据状态", "出票日期", "到期日期", "收票日期", "户名", "名称", "账号", "PK_REGISTER", "票据编号") AS 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FORCE VIEW "NC06192"."VIEW_JL_ARR" ("到货单子表ID", "到货单主表ID", "仓库编码", "仓库名称", "存货编码", "存货名称", "规格型号", "计量单位", "应到数量", "合格入库数量", "批号", "包装规格", "到货单号", "单据日期", "供应商编码", "供应商名称", "原币无税单价", "原币含税单价", "采购订单号", "采购订单子表ID", "采购类型", "税率", "联系电话", "部门编码", "业务员编码", "业务员名称", "备注") AS 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FORCE VIEW "NC06192"."VIEW_JL_CUSTOMER" ("客商编码", "客商名称", "客商简称", "助记码", "是否国外", "CVENDEFINE1", "CVENDEFINE2") AS 
  (
  SELECT code AS 客商编码,
    name       AS 客商名称,
    shortname  AS 客商简称,
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FORCE VIEW "NC06192"."VIEW_JL_INV" ("存货编码", "CINVADDCODE", "存货名称", "规格型号", "是否销售", "是否外购", "是否自制", "助记码", "是否计量", "计量单位名称", "入库超额上限") AS 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FORCE VIEW "NC06192"."VIEW_JL_SUPPLIER" ("客商编码", "客商名称", "客商简称", "助记码", "是否国外", "CVENDEFINE1", "CVENDEFINE2") AS 
  (
  SELECT code AS 客商编码,
    name       AS 客商名称,
    shortname  AS 客商简称,
