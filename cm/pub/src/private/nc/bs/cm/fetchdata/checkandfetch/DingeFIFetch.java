...
/**
 * 定额取数--取存货核算的产成品入库单,生成消耗单 成本生单 1.外系统取数设置，增加页签，定额，页明细 核算要素编码，核算要素名称
 * 2.外系统取数据，增加两列：核算要素编码，核算要素名称
 * 3.取数:新增核算要素取数据，规则如下：根据会计期间，查询存货核算的产成品入库单，取主数量*乘以核算要素的量
 * 具体取哪些核算要素，根据外系统取数设置+成本BOM确定 具体取哪些交易类型的产成品入库单，根据外系统取数设置的完工入库来确定 4.计算完成，生成消耗单.
 */
public class DingeFIFetch extends MaterialOutFIFetch {
	...
	private CMDataVO[] queryCMDataVO(GetDataPara para, FetchParamVO fetchPara) {
		// TODO 自动生成的方法存根
		PullDataStateVO pullData = fetchPara.getPullDataStateVOArr()[0];
...
//		}
		...
		pricesql.append(" inner JOIN sca_bom bom ON fee.ccmbomid = bom.ccmbomid ");// 成本BOM
		pricesql.append(" inner join  cm_costobject costo");// 成本对象 ：物料和自由属性都相同
		pricesql.append(" on (bom.pk_org= costo.pk_org and bom.cmaterialid = costo.cmaterialid    ");
		...
		pricesql.append(" and bom.pk_org", this.pkOrg);
		//成本对象
		pricesql.append(" and costo.ccostobjectid", pullData.getPk_costobject());
		//核算要素
		pricesql.append(" and fee.celementid",pullData.getPk_factor());
    ...
    private void reCalcItemPrice(StuffAggVO[] aggVOs) {
        ...
        for (StuffAggVO aggvo : aggVOs) {
            CircularlyAccessibleValueObject[] cavoArr = aggvo.getAllChildrenVO();
            for (CircularlyAccessibleValueObject cavo : cavoArr) {
                ...
                UFDouble ndiff = CMNumberUtil2.sub(vo.getNmoney(), vo.getNplanmoney());
                if (vo.getNmoney() != null && vo.getNnum() != null && vo.getNnum().compareTo(UFDouble.ZERO_DBL) != 0) {
                ...
                if (vo.getNplanmoney() != null && vo.getNnum() != null
                    ...
                    nplanprice = CMNumberUtil2.div(vo.getNplanmoney(), vo.getNnum());
                }
                ...
                if (CMValueCheck.isNotEmpty(nplanprice)) {
                    nplanprice =
                            nplanprice.setScale(this.scaleutil.getScaleVO().getPriceScale(), UFDouble.ROUND_HALF_UP);
                }
	...
	protected List<Map<IFetchData, PullDataErroInfoVO>> getErrorInfo(
        ...
        Map<IFetchData, PullDataErroInfoVO> clckMap =
                super.getErrorInfoAndFilteredData(datas,
                        new CLCKCheckStrategy(this.pkOrg, this.pkGroup, this.costDomain, this.acountBook), cperiod,
                        isCheckFlag).get(0);
	...
	protected void setHeadVOSepcailValue(FetchParamVO paramvo,
        ...
        stuffHeadVO.setIsourcetype(Integer.valueOf(String.valueOf(CMSourceTypeEnum.IA_PRODUCT_IN.value())));
