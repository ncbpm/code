...
public class JianyanFIFetch extends  AbstractCheckAndFetch<IMMFetchData> {
    ...
    @Override
    public CircularlyAccessibleValueObject[] getData4OutSystem(FetchParamVO paramvo, boolean isCheckFlag)
    ...
    /**
     * 生产制造接口，得到生产制造：生产报告和工序完工报告汇总Map<wkid, MMFetchDataVO>
     *
     * @param paramvo
     *            FetchParamVO 取数参数
     ...
     *             BusinessException'
     */
    public CircularlyAccessibleValueObject[] getFinishOrGxFinish(FetchParamVO paramvo) throws BusinessException {
        ...
        if (FetchDataSchemaEnum.PERIODSCHEMA.equalsValue(fetchscheme)) {
            ...
            costCenterid = Arrays.asList(FIAdapter.getAllCostCenterByOrg(this.pkOrg));
            this.setWkidAndDept(wkids, deptids, costCenterid);
        }

        // 进行取数:工作工作中心为空时,按照部门进行取数
        CircularlyAccessibleValueObject[] mmfetchDataVOs =
                this.getMMDataOfOutSystem(beginDay, endDay, this.pkOrg, this.pkGroup, wkids.toArray(new String[0]),
     ...
     * @return 拉平后的vo数组
     */
    public CircularlyAccessibleValueObject[] getMMDataOfOutSystem(UFDate beginDay, UFDate endDay, String pkOrg,
            String pkGroup, String[] wkids, String[] deptids) throws BusinessException {
        return MMAdapter.getWorkReport4FetchData(beginDay, endDay, pkOrg, pkGroup, wkids, deptids);

    }

    @Override
    public void transVOInserDB(IMMFetchData[] correctData, FetchParamVO paramvo) throws BusinessException {
        Map<String, List<IMMFetchData>> cachedVOMap = super.transVO(correctData, new AbstractFetchStrategy() {
            // 根据key字段，分组同样key值的vo，存到cachedVOMap，作业消耗单以标题 消耗成本中心、成本对象进行区分
            @Override
        ...
        ActivityNumAggVO[] ActivityNumAggVO = this.transActActNumVO(cachedVOMap, correctData, paramvo);
        this.insertByGroup(ActivityNumAggVO);
    }

    // 分批插入作业统计单中
    private void insertByGroup(ActivityNumAggVO[] activityNumAggVO) throws BusinessException {
        ...
        for (ActivityNumAggVO vo : activityNumAggVO) {
            ...
            if (needDealList.size() == 1000) {
                this.getActivityNumPubService().mmCreateBill(needDealList.toArray(new ActivityNumAggVO[0]));
                needDealList.clear();
            }
        }
     ...
     *             BusinessException
     */
    public ActivityNumAggVO[] transActActNumVO(Map<String, List<IMMFetchData>> cachedVOMap, IMMFetchData[] correctData,
            final FetchParamVO paramvo) throws BusinessException {
        // 传给processValues方法的VO必须是初始化的
        ActivityNumAggVO[] ActivityNumAggVO = Constructor.construct(ActivityNumAggVO.class, cachedVOMap.size());
        return super.processValues(cachedVOMap, ActivityNumAggVO,
                new IGroupStrategy<ActivityNumAggVO, ActivityNumHeadVO, ActivityNumItemVO, IMMFetchData>() {
            @Override
            public ActivityNumHeadVO getHeadVO(List<IMMFetchData> volist) {
                IMMFetchData[] datavos = volist.toArray(new IMMFetchData[0]);
                return JianyanFIFetch.this.setActNumHeadVO(paramvo, datavos);
            }
            ...
            public String[] getSumAttrs(ActivityNumItemVO vo) {
                return new String[] {
                        ActivityNumItemVO.NNUM
                }; // 作业量
            }

            @Override
            public ActivityNumItemVO[] getItemVO(List<IMMFetchData> voList) {

                List<ActivityNumItemVO> listItemvo = new ArrayList<ActivityNumItemVO>();

                return JianyanFIFetch.this.setActNumItemVO(voList.toArray(new IMMFetchData[0]), listItemvo);
            }
        });
    }

    @Override
    protected List<Map<IFetchData, PullDataErroInfoVO>> getErrorInfo(IMMFetchData[] datas, String cperiod,
            boolean isCheckFlag) throws BusinessException {
        return super.getErrorInfoAndFilteredData(datas, new MMFetchCheckStrategy(this.pkOrg), cperiod, isCheckFlag);
    }
    ...
    protected CostObjectGenerateVO[] getAndSetCostOjbParam(IFetchData[] datas, AbstractCheckStrategy strategy,
        ...
        for (int i = 0; i < datas.length; i++) {
            ...
            costObjectGenerateVOs[i].setFinstoragetype(data.getFinstoragetype());
            ...
            costObjectGenerateVOs[i].setFproducttype(Integer.valueOf(ProductTypeEnum.MAINPRODUCT.value().toString()));// 主产品标示
     ...
     */
    public ActivityNumHeadVO setActNumHeadVO(FetchParamVO paramvo, IMMFetchData[] datavos) {
     ...
     * @return ActivityNumHeadVO
     */
    public ActivityNumItemVO[] setActNumItemVO(IMMFetchData[] vos, List<ActivityNumItemVO> listItemvo) {
        for (IMMFetchData vo : vos) {
            ActivityNumItemVO ActivityNumItemVO = new ActivityNumItemVO();
            // 当通过工作中心获取不到成本中心时,通过部门获取成本中心
            if (CMStringUtil.isNotEmpty(vo.getWkid()) && super.wkidAndCostCenridMap.get(vo.getWkid()) != null) {
                ActivityNumItemVO.setCcostcenterid(super.wkidAndCostCenridMap.get(vo.getWkid()));
            }
            else {
                ActivityNumItemVO.setCcostcenterid(super.wkidAndCostCenridMap.get(vo.getCdeptid()));
            }
            ...
            ActivityNumItemVO.setIstatus(Integer.valueOf(String.valueOf(CMAllocStatusEnum.UNALLOCATE.value())));
    ...
    public void deleteBill(FetchParamVO paramVOs) throws BusinessException {
        if (FetchDataSchemaEnum.PERIODSCHEMA.equalsValue(paramVOs.getIfetchscheme())) {
            this.getActivityNumPubService().deleteActNumsData4FetchData(paramVOs, true);
        }
        else {
            this.getActivityNumPubService().deleteActNumsData4FetchData(paramVOs, false);
        }
    ...
    protected IActivityNumRewriteForMMFetch getActivityNumPubService() {
        if (this.activityNumPubService == null) {
            this.activityNumPubService = NCLocator.getInstance().lookup(IActivityNumRewriteForMMFetch.class);
        }
    ...
    public IFetchData[] transDatas(CircularlyAccessibleValueObject[] datas, FetchParamVO paramVo) {
        ...
        for (CircularlyAccessibleValueObject cadata : datas) {
            ...
            // 入库类型
            vo.setFinstoragetype(this.getFinstoragetype(data.getFinstoragetype()));
            // 设置类型，区别错误信息和警告信息
            ...
            vo.setResourcetype(Integer.valueOf(CMSourceTypeEnum.PRO_REPORT.getEnumValue().getValue()));// 完工报告->变更为：生产制造
            // 物料辅助属性
